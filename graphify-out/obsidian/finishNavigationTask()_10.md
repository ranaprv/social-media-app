---
source_file: "frontend/.next-old/dev/static/chunks/node_modules_next_dist_client_0r5nbpw._.js"
type: "code"
community: "Abortpendingcachenode()"
location: "L9317"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Abortpendingcachenode
---

# finishNavigationTask()

## Connections
- [[abortRemainingPendingTasks()_10]] - `calls` [EXTRACTED]
- [[dispatchRetryDueToTreeMismatch()_10]] - `calls` [EXTRACTED]
- [[node_modules_next_dist_client_0r5nbpw._.js]] - `contains` [EXTRACTED]
- [[spawnDynamicRequests()_10]] - `calls` [EXTRACTED]
- [[waitForRequestsToFinish()_10]] - `calls` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Abortpendingcachenode