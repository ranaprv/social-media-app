---
source_file: "frontend/.next-old/dev/server/chunks/ssr/node_modules_1br2cfr._.js"
type: "code"
community: "Node Modules 1Br2Cfr"
location: "L4705"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Node_Modules_1Br2Cfr
---

# fillInFallbackFlightRouterState()

## Connections
- [[createInitialRSCPayloadFromFallbackPrerender()_6]] - `calls` [EXTRACTED]
- [[fillInFallbackFlightRouterStateImpl()_6]] - `calls` [EXTRACTED]
- [[node_modules_1br2cfr._.js]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Node_Modules_1Br2Cfr